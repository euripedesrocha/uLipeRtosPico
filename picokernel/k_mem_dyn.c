...
#if (K_HEAP_SIZE <= 32768)
	#define MAX_FLI                             (15)
	#define MAX_LOG2_SLI                        (4)
	#define MAX_SLI                             (1 << MAX_LOG2_SLI)
#elif (K_HEAP_SIZE > 32768) &&(K_HEAP_SIZE < 65536)
	#define MAX_FLI                             (16)
	#define MAX_LOG2_SLI                        (4)
	#define MAX_SLI                             (1 << MAX_LOG2_SLI)
#else
...
/** private variables */
static uint8_t kernel_heap[K_HEAP_SIZE+sizeof(tlsf_t)] = { 0 };
...
static __inline int32_t ls_bit(int32_t i) {
    return ((i == 0 )? 0 : 31 - port_bit_ls_scan(i));
}

static __inline int32_t ms_bit(int32_t i) {
    return ((i == 0)? 0 : 31 - port_bit_fs_scan(i));
}
...
{
	size_t s = K_HEAP_SIZE + sizeof(tlsf_t);
	s = init_memory_pool(s, kernel_heap);
	ulipe_assert(s != 0);
}
...
void *k_malloc(size_t size)
{
    ...
    if (size > K_HEAP_SIZE) size = (K_HEAP_SIZE);

    /* request memory from the allocator block. */
    ret = malloc_ex(size, kernel_heap);
