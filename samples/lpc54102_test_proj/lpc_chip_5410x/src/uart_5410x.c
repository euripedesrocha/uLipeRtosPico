...
int Chip_UART_Send(LPC_USART_T *pUART, const void *data, uint32_t numBytes)
{
	int sent = 0;
	uint8_t *p8 = (uint8_t *) data;

	/* Send until the transmit FIFO is full or out of bytes */
	while ((sent < numBytes) && ((Chip_UART_GetStatus(pUART) & UART_STAT_TXRDY) != 0)) {
...
int Chip_UART_SendBlocking(LPC_USART_T *pUART, const void *data, uint32_t numBytes)
{
	uint32_t pass, sent = 0;
	uint8_t *p8 = (uint8_t *) data;
...
int Chip_UART_Read(LPC_USART_T *pUART, void *data, uint32_t numBytes)
{
...
int Chip_UART_ReadBlocking(LPC_USART_T *pUART, void *data, uint32_t numBytes)
{
	int pass, readBytes = 0;
	volatile int tmp = numBytes;
...
void Chip_UART_SetBaud(LPC_USART_T *pUART, uint32_t baudrate)
{
	...
	baud.clk = 	Chip_Clock_GetAsyncSyscon_ClockRate();	/* Clock frequency */
	baud.baud = baudrate;	/* Required baud rate */
	baud.ovr =  8;	/* Set the oversampling to the recommended rate */
	baud.mul = baud.div = 0;
	Chip_UART_CalcBaud(pUART, &baud);
